...
// onMounted(() => store.getVehicleDetail(route.params.id))
watchEffect(async () => {
  await store.getVehicleDetail(route.params.id)
})
</script>

<template>
  <form @submit.prevent="store.handleUpdate(route.params.id)" novalidate>
